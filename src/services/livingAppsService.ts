...
import type { Ladeprotokoll, CreateLadeprotokoll } from '@/types/app';
...
export class LivingAppsService {
  // --- LADEPROTOKOLL ---
  static async getLadeprotokoll(): Promise<Ladeprotokoll[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.LADEPROTOKOLL}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Ladeprotokoll[];
    return enrichLookupFields(records, 'ladeprotokoll');
  }
  static async getLadeprotokollEntry(id: string): Promise<Ladeprotokoll | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.LADEPROTOKOLL}/records/${id}`);
    const record = { record_id: data.id, ...data } as Ladeprotokoll;
    return enrichLookupFields([record], 'ladeprotokoll')[0];
  }
  static async createLadeprotokollEntry(fields: CreateLadeprotokoll) {
    return callApi('POST', `/apps/${APP_IDS.LADEPROTOKOLL}/records`, { fields: cleanFieldsForApi(fields as any, 'ladeprotokoll') });
  }
  static async updateLadeprotokollEntry(id: string, fields: Partial<CreateLadeprotokoll>) {
    return callApi('PATCH', `/apps/${APP_IDS.LADEPROTOKOLL}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'ladeprotokoll') });
  }
  static async deleteLadeprotokollEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.LADEPROTOKOLL}/records/${id}`);
  }
