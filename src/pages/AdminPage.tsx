...
const LADEPROTOKOLL_FIELDS = [
  { key: 'lampenname', label: 'Name der Lampe', type: 'string/text' },
  { key: 'lampentyp', label: 'Lampentyp', type: 'lookup/select', options: [{ key: 'taschenlampe', label: 'Taschenlampe' }, { key: 'stirnlampe', label: 'Stirnlampe' }, { key: 'campinglampe', label: 'Campinglampe' }, { key: 'gartenlampe', label: 'Gartenlampe' }, { key: 'arbeitsleuchte', label: 'Arbeitsleuchte' }, { key: 'sonstige', label: 'Sonstige' }] },
  { key: 'standort', label: 'Standort / Aufbewahrungsort', type: 'string/text' },
  { key: 'notizen', label: 'Notizen zur Lampe', type: 'string/textarea' },
  { key: 'letztes_aufladen', label: 'Zuletzt aufgeladen am', type: 'date/datetimeminute' },
  { key: 'akkustand', label: 'Akkustand nach dem Laden', type: 'lookup/radio', options: [{ key: 'voll', label: 'Voll geladen (100 %)' }, { key: 'teilweise', label: 'Teilweise geladen (50–99 %)' }, { key: 'niedrig', label: 'Niedrig geladen (unter 50 %)' }] },
  { key: 'naechstes_laden', label: 'Nächstes geplantes Aufladen', type: 'date/date' },
  { key: 'laden_vollstaendig', label: 'Ladevorgang vollständig abgeschlossen', type: 'bool' },
];

const ENTITY_TABS = [
  { key: 'ladeprotokoll', label: 'Ladeprotokoll', pascal: 'Ladeprotokoll' },
] as const;
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('ladeprotokoll');
  const [selectedIds, setSelectedIds] = useState<Record<EntityKey, Set<string>>>(() => ({
    'ladeprotokoll': new Set(),
  }));
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'ladeprotokoll': return (data as any).ladeprotokoll as Ladeprotokoll[] ?? [];
      default: return [];
    }
  }, [data]);
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'ladeprotokoll': return LADEPROTOKOLL_FIELDS;
      default: return [];
    }
  }, []);
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'ladeprotokoll': return {
        create: (fields: any) => LivingAppsService.createLadeprotokollEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateLadeprotokollEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteLadeprotokollEntry(id),
      };
      default: return null;
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'ladeprotokoll' || dialogState?.entity === 'ladeprotokoll') && (
        <LadeprotokollDialog
          open={createEntity === 'ladeprotokoll' || dialogState?.entity === 'ladeprotokoll'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'ladeprotokoll' ? handleUpdate : (fields: any) => handleCreate('ladeprotokoll', fields)}
          defaultValues={dialogState?.entity === 'ladeprotokoll' ? dialogState.record?.fields : undefined}
          enablePhotoScan={AI_PHOTO_SCAN['Ladeprotokoll']}
          enablePhotoLocation={AI_PHOTO_LOCATION['Ladeprotokoll']}
        />
      )}
      {viewState?.entity === 'ladeprotokoll' && (
        <LadeprotokollViewDialog
          open={viewState?.entity === 'ladeprotokoll'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'ladeprotokoll', record: r }); }}
        />
      )}
